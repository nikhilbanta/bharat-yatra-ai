...
def render_login():
    ...
    with st.form("login_form"):
        identifier = st.text_input("Name or email", placeholder="e.g. priya@example.com")
...
def render_profile_form(existing: dict | None):
    ...
    with st.form("profile_form"):
        ...
        with col1:
            ...
            location = st.text_input("Home city/state", value=existing.get("location", ""))
        with col2:
            ...
            interests = st.multiselect("Interests", INTERESTS_OPTIONS, default=existing.get("interests", []))
        ...
        if submitted:
            ...
            db.save_profile(_identifier_key(), profile)
...
def render_recommender(profile: dict):
    ...
    if st.button("Get recommendations", key="rec_btn") and query:
        with st.spinner("Finding destinations that match your profile..."):
            try:
                ...
                db.add_history_entry(_identifier_key(), "recommendation", query, result.text)
            except GeminiClientError as e:
                st.error(f"Could not generate recommendations right now: {e}")


def render_hidden_gems(profile: dict):
    ...
    if st.button("Find hidden gems", key="gems_btn") and location:
                ...
                if result.sources:
                    with st.expander("Sources"):
                        for s in result.sources:
                            st.write(s)
                db.add_history_entry(_identifier_key(), "hidden_gems", location, result.text)
            except GeminiClientError as e:
                st.error(f"Could not search for hidden gems right now: {e}")


def render_storytelling(profile: dict):
    ...
    if st.button("Tell me the story", key="story_btn") and place:
        with st.spinner("Weaving the story..."):
            try:
                system, user_prompt = build_storytelling_prompt(place, profile)
                result = generate(user_prompt, system_instruction=system)
                st.markdown(result.text)
                db.add_history_entry(_identifier_key(), "storytelling", place, result.text)
            except GeminiClientError as e:
                st.error(f"Could not generate the story right now: {e}")


def render_events_experiences(profile: dict):
    ...
    if st.button("Find events & experiences", key="events_btn") and location:
            ...
            try:
                ...
                if result.sources:
                    with st.expander("Sources"):
                        for s in result.sources:
                            st.write(s)
                db.add_history_entry(_identifier_key(), "events_experiences", location, result.text)
            except GeminiClientError as e:
...
def render_history():
    ...
    entries = db.get_history(_identifier_key(), limit=20)
    if not entries:
...
def main():
    ...
    profile = db.get_profile(_identifier_key())
